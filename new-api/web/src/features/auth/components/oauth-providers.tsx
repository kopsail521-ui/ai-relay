...
type OAuthProvidersProps = {
  ...
  disabled?: boolean
  /** Return false to cancel OAuth click (e.g. legal consent). Buttons stay enabled. */
...
export function OAuthProviders({
  ...
  const run = (fn: () => void) => {
    if (onBeforeOAuth && onBeforeOAuth() === false) return
    fn()
  }
  ...
  if (status?.discord_oauth) {
    providerButtons.push({
      ...
      onClick: () => run(handleDiscordLogin),
  ...
  if (status?.linuxdo_oauth) {
    providerButtons.push({
      ...
      onClick: () => run(handleLinuxDOLogin),
  ...
  if (status?.telegram_oauth) {
    providerButtons.push({
      ...
      onClick: () => run(handleTelegramLogin),
  ...
  if (customProviders && customProviders.length > 0) {
    for (const provider of customProviders) {
      providerButtons.push({
        ...
        onClick: () => run(() => handleCustomOAuthLogin(provider)),
      })
    }
  }
