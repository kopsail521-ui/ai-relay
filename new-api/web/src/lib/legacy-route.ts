...
export function resolveLegacyRoute(rawHref: string): string | null {
  ...
  // Public marketplace lives at /pricing. Keep /models → /pricing for old links.
  if (pathname === '/models' || pathname.startsWith('/models/')) {
    const suffix = pathname.slice('/models'.length)
    return buildTargetHref(`/pricing${suffix}`, source)
  }
