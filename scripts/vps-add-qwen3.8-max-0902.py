#!/usr/bin/env python3
"""Add OpenLux model qwen3.8-max-0902 (sell = OpenLux cost ×2.5) into New API sqlite."""
import json
import os
import sqlite3
import sys
...
OUR_MODEL_RATIO = 0.75  # sell_in $1.5 / MTok → ratio ($2/MTok baseline)
OUR_COMPLETION_RATIO = 3
TAG = "大语言模型"
VENDOR_CANDIDATES = ("阿里巴巴", "Alibaba", "Qwen", "通义")
ICON = "Qwen.Color"
ENDPOINTS = json.dumps({"openai": "/v1/chat/completions"}, separators=(",", ":"))
DESC = "Qwen3.8 Max（0902）旗舰对话与推理模型 · 约 $1.5 / $4.5 每百万 tokens"
...
def main():
    ...
    vid = None
    for vname in VENDOR_CANDIDATES:
        cur.execute("SELECT id FROM vendors WHERE name=?", (vname,))
        vrow = cur.fetchone()
        if vrow:
            vid = vrow[0]
            print("vendor", vname, vid)
            break
    if vid is None:
        # create Alibaba vendor if missing
    ...
    print("DONE_ADD_QWEN38_MAX_0902")
