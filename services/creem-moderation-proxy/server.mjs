...
function buildBillingUnitScript() {
  // Detail drawers + pricing list cards. v8: list badges/suffix for per-second models.
  const js = `(function(){if(window.__keyoBillV8)return;window.__keyoBillV8=1;
...
function rewrite(root,u){if(!root||!u||u.unit==="request")return;var badge=L(u.badge)||u.badge_zh||"按秒计费";var pkey=L(u.price_key)||u.price_key_zh||"每秒";var suf=L(u.suffix)||u.suffix_zh||"/秒";var tw=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,null);var n;while(n=tw.nextNode()){var t=(n.nodeValue||"").replace(/\\s+/g," ").trim();if(!t)continue;if(t==="按次计费"||t==="按次計費"||t==="Per Request"||t==="Per-call")n.nodeValue=badge;else if(t==="每次请求"||t==="每次請求"||t==="Per request")n.nodeValue=pkey;else if(t==="$/请求"||t==="$/請求"||t==="$/request"||t==="$ / request")n.nodeValue="$"+suf.replace(/^\\//,"").replace(/^ /,"/");else if(t==="/请求")||t==="/請求"||t==="/request")n.nodeValue=suf}}
function rewriteListCards(){try{var map=MAP();var ids=Object.keys(map).filter(function(k){return k&&k.indexOf("__")!==0&&map[k]&&map[k].unit&&map[k].unit!=="request"});if(!ids.length)return;var cards=document.querySelectorAll("article,[class*=Card],[class*=card],li,div");for(var i=0;i<cards.length;i++){var el=cards[i];if(!el||el.children&&el.children.length>40)continue;var tx=(el.innerText||"").slice(0,800);if(!tx||tx.length<8)continue;var mid=null;for(var k=0;k<ids.length;k++){if(tx.indexOf(ids[k])>=0){mid=ids[k];break}}if(!mid)continue;var u=map[mid];if(!u)continue;rewrite(el,u)}}catch(e){}}
...
function tick(){try{rewriteListCards();var roots=detailRoots();for(var i=0;i<roots.length;i++){var root=roots[i];var mid=detect(root);if(!mid)continue;var u=meta(mid);if(!u||!u.price_table)continue;rewrite(root,u);mountTable(root,mid,u);fixDesc(root,u)}}catch(e){}}
