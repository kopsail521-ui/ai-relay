...
function buildBillingUnitScript() {
  // Detail pages + cards: New API ModelPrice always shows as 按次/每次请求.
  // Overlay rewrites labels when marketplace copy unit is second/page/char.
  const js = `(function(){if(window.__keyoBillingUnitV2)return;window.__keyoBillingUnitV2=1;function map(){return window.__KEYO_MKT_COPY||{}}var LANGS=["zhCN","zhTW","en","fr","ru","ja","vi"];function langCode(){try{var v=(localStorage.getItem("i18nextLng")||"").trim().replace(/_/g,"-");var l=v.toLowerCase();if(LANGS.indexOf(v)>=0)return v;if(l==="zhcn"||l==="zh-cn"||l==="zh-hans"||l==="zh")return "zhCN";if(l==="zhtw"||l==="zh-tw"||l==="zh-hk"||l==="zh-mo"||l.indexOf("zh-hant")===0)return "zhTW";if(l.indexOf("ja")===0)return "ja";if(l.indexOf("fr")===0)return "fr";if(l.indexOf("ru")===0)return "ru";if(l.indexOf("vi")===0)return "vi";if(l.indexOf("en")===0)return "en";return "zhCN"}catch(e){return "zhCN"}}function unitOf(n){return map()[n]||null}function label(u,field){var code=langCode();var bag=u[field]||{};if(typeof bag==="string")return bag;return bag[code]||bag.zhCN||bag.en||""}function detectModel(){try{var path=location.pathname||"";var m=path.match(/\\/(?:pricing|models)\\/(?:detail\\/)?([^\\/?#]+)/i);if(m){var id=decodeURIComponent(m[1]);if(unitOf(id))return id}var keys=Object.keys(map()).filter(function(k){return k!=="__tags__"});var hs=document.querySelectorAll("h1,h2,h3,[data-model],title");for(var i=0;i<hs.length;i++){var t=(hs[i].textContent||"").replace(/\\s+/g," ").trim();if(unitOf(t))return t;for(var j=0;j<keys.length;j++){if(t===keys[j]||t.indexOf(keys[j])===0)return keys[j]}}}catch(e){}return null}function applyIn(scope,u){if(!scope||!u)return;if(u.unit==="request"||u.unit==="token")return;var badge=label(u,"badge");var pkey=label(u,"price_key");var suf=label(u,"suffix");var walk=scope.querySelectorAll("*");for(var j=0;j<walk.length;j++){var n=walk[j];if(n.children&&n.children.length)continue;var t=(n.textContent||"").replace(/\\s+/g," ").trim();if(!t)continue;if(/^(按次计费|按次計費|Per Request|Par requête|За запрос|リクエスト課金|Theo lần gọi|Per-call|Usage-based)$/.test(t)){if(badge)n.textContent=badge;continue}if(/^(每次请求|每次請求|Per request|Par requête|За запрос|リクエストごと|Mỗi lần gọi)$/.test(t)){if(pkey)n.textContent=pkey;continue}if(/^\\/\\s*(request|次|回|lần|requête|запрос)$/i.test(t)||/\\/\\s*(request|次)$/i.test(t)){if(suf)n.textContent=suf;continue}}}function climb(el){var c=el;for(var i=0;i<16&&c&&c.parentElement;i++){var p=c.parentElement;var cls=(p.getAttribute&&p.getAttribute("class"))||"";if(p.children&&p.children.length>=2&&/(grid|flex|card)/i.test(String(cls)))return c;c=p}return el.parentElement}function fix(){try{var mid=detectModel();if(mid){var u=unitOf(mid);var root=document.querySelector("main")||document.body;applyIn(root,u)}var nodes=document.querySelectorAll("h1,h2,h3,h4,h5,div,span,p,a,button,label");for(var i=0;i<nodes.length;i++){var el=nodes[i];if(el.children&&el.children.length>2)continue;var name=(el.textContent||"").replace(/\\s+/g," ").trim();var uu=unitOf(name);if(!uu||uu.unit==="request"||uu.unit==="token")continue;var card=climb(el);var scope=(card&&card.parentElement)||card||el.parentElement;applyIn(scope,uu)}}catch(e){}}setInterval(fix,500);document.addEventListener("click",function(){setTimeout(fix,80)},true);window.addEventListener("popstate",function(){setTimeout(fix,80)})})();`;
  return "<script>" + js + "</script>";
}
...
async function proxyRequest(req, res, bodyBuf) {
  ...
  if (isSpaShell && buf.length > 0) {
    try {
      ...
      if (!html.includes("keyo-pricing-sort-v7")) {
        html = html
          .replace(/<!--keyo-pricing-sort(?:-v\d+)?-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-billing-unit-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-locale-desc-->[\s\S]*?<\/script>/g, "");
        const inject = `<!--keyo-pricing-sort-v7-->${PRICING_SORT_SCRIPT}<!--keyo-locale-desc-->${LOCALE_DESC_SCRIPT}<!--keyo-billing-unit-->${BILLING_UNIT_SCRIPT}`;
        if (html.includes("<head>")) {
