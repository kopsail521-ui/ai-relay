...
function buildBillingUnitScript() {
  const js = `(function(){if(window.__keyoBillV4)return;window.__keyoBillV4=1;
...
function detect(){try{var keys=Object.keys(MAP()).filter(function(k){return k!=="__tags__"&&MAP()[k]&&MAP()[k].price_table});var body=(document.body&&document.body.innerText)||"";for(var i=0;i<keys.length;i++){if(body.indexOf(keys[i])>=0)return keys[i]}var hs=document.querySelectorAll("h1,h2,h3");for(var j=0;j<hs.length;j++){var t=(hs[j].textContent||"").trim();for(var k=0;k<keys.length;k++){if(t===keys[k]||t.indexOf(keys[k])>=0)return keys[k]}}}catch(e){}return null}
...
function mountTable(mid,u){if(!u||!u.price_table||!u.price_table.rows)return;var id="keyo-pt-"+mid.replace(/[^\\w.-]/g,"_");if(document.getElementById(id))return;var anchor=null;var nodes=document.querySelectorAll("h2,h3,h4,div,span,p");for(var i=0;i<nodes.length;i++){var el=nodes[i];var t=(el.textContent||"").replace(/\\s+/g," ").trim();if(t==="基础价格"||t==="基礎價格"||t==="Base Price"||t==="定价"||t==="Pricing"){anchor=el.closest("section")||el.parentElement;break}}if(!anchor){var hits=document.querySelectorAll("div,span");for(var j=0;j<hits.length;j++){var h=hits[j];var ht=(h.textContent||"").replace(/\\s+/g," ").trim();if(ht==="每次请求"||ht==="Per request"){anchor=h.closest("section")||(h.parentElement&&h.parentElement.parentElement);break}}}if(!anchor)return;var wrap=document.createElement("div");wrap.id=id;wrap.style.cssText="margin:14px 0 8px;overflow:auto;border:1px solid rgba(127,127,127,.25);border-radius:12px";var table=document.createElement("table");table.style.cssText="width:100%;border-collapse:collapse;font-size:13px;line-height:1.4";var thead=document.createElement("thead");var trh=document.createElement("tr");colsOf(u.price_table).forEach(function(c){var th=document.createElement("th");th.textContent=c;th.style.cssText="text-align:left;padding:11px 12px;background:rgba(127,127,127,.08);border-bottom:1px solid rgba(127,127,127,.2);white-space:nowrap";trh.appendChild(th)});thead.appendChild(trh);table.appendChild(thead);var tb=document.createElement("tbody");u.price_table.rows.forEach(function(row){var tr=document.createElement("tr");row.forEach(function(cell,idx){var td=document.createElement("td");td.textContent=cell;td.style.cssText="padding:10px 12px;border-bottom:1px solid rgba(127,127,127,.12)"+(idx===row.length-1?";font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-weight:600":"");tr.appendChild(td)});tb.appendChild(tr)});table.appendChild(tb);wrap.appendChild(table);anchor.appendChild(wrap);try{anchor.querySelectorAll("div").forEach(function(row){if(row.id===id||row.querySelector("#"+id))return;var tt=(row.textContent||"").replace(/\\s+/g," ");if((/每次请求|Per request/).test(tt)&&(/USD\\s*\\d|\\$\\s*\\d/).test(tt)&&row.children.length<=5){row.style.display="none"}})}catch(e){}}
function tick(){try{var mid=detect();if(!mid)return;var u=meta(mid);if(!u)return;var roots=[document.body];document.querySelectorAll("[role=dialog],[data-radix-portal],[data-state=open]").forEach(function(el){roots.push(el)});roots.forEach(function(r){rewrite(r,u)});mountTable(mid,u);try{var want=L(u.descriptions)||u.description_zh||u.description;if(want){var ps=document.querySelectorAll("p,div");for(var i=0;i<ps.length;i++){var el=ps[i];if(el.children&&el.children.length>1)continue;var t=(el.textContent||"").replace(/\\s+/g," ").trim();if(!t||t.length<20||t.length>400)continue;if(/bill \\(ref|per second|USD 0\\.|\\/ sec|参考视频|按秒：/.test(t)||(t.indexOf("Seedance")>=0&&t.indexOf("video")>=0)){if(t!==want)el.textContent=want;break}}}}catch(e){}}catch(e){}}
setInterval(tick,300);try{new MutationObserver(function(){tick()}).observe(document.documentElement,{childList:true,subtree:true})}catch(e){}
...
async function proxyRequest(req, res, bodyBuf) {
  ...
  if (isSpaShell && buf.length > 0) {
    try {
      ...
      if (!html.includes("keyo-pricing-sort-v9")) {
        html = html
          .replace(/<!--keyo-pricing-sort(?:-v\d+)?-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-billing-unit-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-locale-desc-->[\s\S]*?<\/script>/g, "");
        const inject = `<!--keyo-pricing-sort-v9-->${PRICING_SORT_SCRIPT}<!--keyo-locale-desc-->${LOCALE_DESC_SCRIPT}<!--keyo-billing-unit-->${BILLING_UNIT_SCRIPT}`;
        if (html.includes("<head>")) {
