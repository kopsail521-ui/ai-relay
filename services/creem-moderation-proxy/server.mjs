...
function buildBillingUnitScript() {
  ...
  // v6: find "基础价格" via text nodes (headers often have icons → childNodes>1); insert table after that block.
  const js = `(function(){if(window.__keyoBillV6)return;window.__keyoBillV6=1;
...
function detailRoots(){var out=[];var seen=typeof WeakSet!=="undefined"?new WeakSet():null;function add(el){if(!el)return;if(seen){if(seen.has(el))return;seen.add(el)}else if(out.indexOf(el)>=0)return;out.push(el)}document.querySelectorAll("[role=dialog]").forEach(add);document.querySelectorAll("[data-radix-portal]").forEach(add);document.querySelectorAll("[data-state=open]").forEach(function(el){var t=el.innerText||"";if(/定价|Pricing|基础价格|Base Price|seedance|按秒/.test(t))add(el)});return out}
function detect(root){try{var keys=Object.keys(MAP()).filter(function(k){return k!=="__tags__"&&MAP()[k]&&MAP()[k].price_table});var hs=root.querySelectorAll("h1,h2,h3,[class*=title],button,span,div");for(var j=0;j<hs.length;j++){var t=(hs[j].textContent||"").replace(/\\s+/g," ").trim();if(t.length>80)continue;for(var k=0;k<keys.length;k++){if(t===keys[k]||t.indexOf(keys[k])>=0)return keys[k]}}var body=(root.innerText||"").slice(0,4000);for(var i=0;i<keys.length;i++){if(body.indexOf(keys[i])>=0)return keys[i]}}catch(e){}return null}
...
function climbBlock(el,root,reLabel){var block=el;for(var i=0;i<10&&block&&block!==root;i++){var tx=(block.textContent||"").replace(/\\s+/g," ");if(reLabel.test(tx)&&/(\\$|USD|每秒|Per second|每次)/.test(tx)&&block.children&&block.children.length)return block;block=block.parentElement}return el&&el.parentElement}
function buildTable(id,u){var wrap=document.createElement("div");wrap.id=id;wrap.setAttribute("data-keyo-price-table","1");wrap.style.cssText="margin:12px 0 10px;overflow:auto;border:1px solid rgba(127,127,127,.28);border-radius:12px";var cap=document.createElement("div");cap.textContent=lang().indexOf("zh")===0?"分规格价目（按秒）":"Price by spec (per second)";cap.style.cssText="padding:10px 12px 6px;font-size:13px;font-weight:600;opacity:.9";wrap.appendChild(cap);var table=document.createElement("table");table.style.cssText="width:100%;border-collapse:collapse;font-size:13px;line-height:1.45";var thead=document.createElement("thead");var trh=document.createElement("tr");colsOf(u.price_table).forEach(function(c){var th=document.createElement("th");th.textContent=c;th.style.cssText="text-align:left;padding:10px 12px;background:rgba(127,127,127,.08);border-bottom:1px solid rgba(127,127,127,.2);white-space:nowrap";trh.appendChild(th)});thead.appendChild(trh);table.appendChild(thead);var tb=document.createElement("tbody");u.price_table.rows.forEach(function(row){var tr=document.createElement("tr");row.forEach(function(cell,idx){var td=document.createElement("td");td.textContent=cell;td.style.cssText="padding:9px 12px;border-bottom:1px solid rgba(127,127,127,.12)"+(idx===row.length-1?";font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-weight:600":"");tr.appendChild(td)});tb.appendChild(tr)});table.appendChild(tb);wrap.appendChild(table);return wrap}
function mountTable(root,mid,u){if(!root||!u||!u.price_table||!u.price_table.rows)return;var id="keyo-pt-"+mid.replace(/[^\\w.-]/g,"_");if(document.getElementById(id))return;var lab=findTextEl(root,["基础价格","基礎價格","Base Price"]);var block=null;if(lab){block=climbBlock(lab,root,/基础价格|基礎價格|Base Price/)}else{lab=findTextEl(root,["定价","Pricing"]);if(lab)block=climbBlock(lab,root,/定价|Pricing/)}if(!block){var per=findTextEl(root,["每秒","每秒","Per second"]);if(per)block=climbBlock(per,root,/每秒|Per second/)}if(!block)return;var wrap=buildTable(id,u);if(block.parentElement){block.parentElement.insertBefore(wrap,block.nextSibling)}else{block.appendChild(wrap)}}
...
setInterval(tick,500);try{new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true})}catch(e){}
...
async function proxyRequest(req, res, bodyBuf) {
  ...
  if (isSpaShell && buf.length > 0) {
    try {
      ...
      if (!html.includes("keyo-pricing-sort-v11")) {
        html = html
          .replace(/<!--keyo-pricing-sort(?:-v\d+)?-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-billing-unit-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-locale-desc-->[\s\S]*?<\/script>/g, "");
        const inject = `<!--keyo-pricing-sort-v11-->${PRICING_SORT_SCRIPT}<!--keyo-locale-desc-->${LOCALE_DESC_SCRIPT}<!--keyo-billing-unit-->${BILLING_UNIT_SCRIPT}`;
        if (html.includes("<head>")) {
