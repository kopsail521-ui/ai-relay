...
function vendorRank(name) {
  ...
  // Exact case-insensitive only — never substring-match (avoids Meta⊂Mistral etc.)
...
function enrichPricingPayload(payload) {
  ...
  // Collapse alias vendors (Qwen→阿里巴巴) but remap model.vendor_id → kept id
  // so sidebar countBy(vendor_name) never goes to 0 for real models.
  const idRemap = new Map(); // droppedVendorId → keptVendorId
  const vendors = Array.isArray(payload.vendors)
    ? (() => {
        const seen = new Map(); // canonName → kept vendor row
        const out = [];
        for (const v of payload.vendors) {
          ...
          const icon =
            key === "字节跳动" && (!v.icon || v.icon === "Custom")
              ? "Doubao.Color"
              : !v.icon || v.icon === "Custom"
                ? ""
                : v.icon;
          if (seen.has(key)) {
            idRemap.set(v.id, seen.get(key).id);
            continue;
          }
          const row = { ...v, name: key, icon };
          seen.set(key, row);
          out.push(row);
        }
        return out;
  ...
  const data = payload.data.map((m) => {
    ...
    let vid = next.vendor_id;
    if (vid != null && idRemap.has(vid)) vid = idRemap.get(vid);
    if (vid != null) next.vendor_id = vid;
    const vrow = vid != null ? idToVendor.get(vid) : null;
    if (vrow) {
      next.vendor_name = vrow.name;
    } else if (next.vendor_name) {
      next.vendor_name = scrubVendorName(next.vendor_name);
    }
...
function pricedIds(){try{return Object.keys(MAP()).filter(function(k){return k&&k.indexOf("__")!==0&&MAP()[k]&&MAP()[k].price_table&&MAP()[k].price_table.rows&&MAP()[k].price_table.rows.length})}catch(e){return[]}}
function detailRoots(){var out=[];var seen=typeof WeakSet!=="undefined"?new WeakSet():null;function add(el){if(!el||!el.querySelector)return;if(seen){if(seen.has(el))return;seen.add(el)}else if(out.indexOf(el)>=0)return;out.push(el)}document.querySelectorAll("[role=dialog],aside,[class*=Sheet],[class*=sheet],[class*=Drawer],[class*=drawer],[class*=Panel],[class*=panel],[data-radix-portal]").forEach(add);document.querySelectorAll("div,section").forEach(function(el){if(!el||(el.children&&el.children.length>80))return;var t=el.innerText||"";if(t.length<40||t.length>8000)return;if(/(基础价格|基礎價格|Base Price)/.test(t)&&/(按分组定价|Grouped Pricing|类型|Type|端点|Endpoints)/.test(t))add(el)});return out}
function detect(root){try{var keys=pricedIds();var hs=root.querySelectorAll("h1,h2,h3,h4,[class*=title],button,span,div,a,code");for(var j=0;j<hs.length;j++){var t=(hs[j].textContent||"").replace(/\\s+/g," ").trim();if(!t||t.length>120)continue;for(var k=0;k<keys.length;k++){if(t===keys[k]||t.indexOf(keys[k])>=0)return keys[k]}}var body=(root.innerText||"").slice(0,8000);for(var i=0;i<keys.length;i++){if(body.indexOf(keys[i])>=0)return keys[i]}}catch(e){}return null}
function rewrite(root,u){if(!root||!u)return;if(u.unit==="request"||u.unit==="page"||u.unit==="character")return;var badge=L(u.badge)||u.badge_zh||"按秒计费";var pkey=L(u.price_key)||u.price_key_zh||"每秒";var suf=L(u.suffix)||u.suffix_zh||"/秒";if(u.unit==="10k_chars"){badge=L(u.badge)||u.badge_zh||"按万字符计费";pkey=L(u.price_key)||u.price_key_zh||"每万字符";suf=L(u.suffix)||u.suffix_zh||"/万字符"}var tw=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,null);var n;while(n=tw.nextNode()){var raw=n.nodeValue||"";var t=raw.replace(/\\s+/g," ").trim();if(!t)continue;if(t==="按次计费"||t==="按次計費"||t==="Per Request"||t==="Per-call"||t==="Per request")n.nodeValue=badge;else if(t==="每次请求"||t==="每次請求"||t==="Per request"||t==="每请求")n.nodeValue=pkey;else if(t==="$/请求"||t==="$/請求"||t==="$/request"||t==="$ / request")n.nodeValue=(suf.charAt(0)==="/"?"$":"$/")+suf.replace(/^\\/?\\s*/,"");else if(t==="/请求"||t==="/請求"||t==="/request")n.nodeValue=suf}}
function rewriteListCards(){try{var map=MAP();var ids=Object.keys(map).filter(function(k){return k&&k.indexOf("__")!==0&&map[k]&&map[k].unit&&map[k].unit!=="request"&&map[k].unit!=="page"&&map[k].unit!=="character"});if(!ids.length)return;var cards=document.querySelectorAll("article,[class*=Card],[class*=card],li,div");for(var i=0;i<cards.length;i++){var el=cards[i];if(!el||(el.children&&el.children.length>50))continue;var tx=(el.innerText||"").slice(0,900);if(!tx||tx.length<8)continue;var mid=null;for(var k=0;k<ids.length;k++){if(tx.indexOf(ids[k])>=0){mid=ids[k];break}}if(!mid)continue;rewrite(el,map[mid])}}catch(e){}}
...
function mountTable(root,mid,u){if(!root||!u||!u.price_table||!u.price_table.rows||!u.price_table.rows.length)return;var id="keyo-pt-"+mid.replace(/[^\\w.-]+/g,"_");var old=document.getElementById(id);if(old){if(root.contains(old))return;try{old.remove()}catch(e){}}var lab=findTextEl(root,["基础价格","基礎價格","Base Price"])||findTextEl(root,["每秒","Per second","每次请求","每次請求"])||findTextEl(root,["定价","Pricing"]);if(!lab){root.appendChild(buildTable(id,u));return}var block=compactPriceBlock(lab,root);var wrap=buildTable(id,u);if(!placeAfter(block,wrap)){var grp=findTextEl(root,["按分组定价","Grouped Pricing","Group Pricing"]);if(grp&&grp.parentElement)grp.parentElement.insertBefore(wrap,grp);else root.appendChild(wrap)}}
function fixDesc(root,u){var want=L(u.descriptions)||u.description_zh||u.description;if(!want)return;var BAD=/bill\\s*\\(ref|Seedance\\s+[\\d.]+\\s+video generation;|USD\\s*0\\.\\d+\\s*\\/\\s*sec/i;var nodes=root.querySelectorAll("p,span,div");for(var i=0;i<nodes.length;i++){var el=nodes[i];if(el.children&&el.children.length)continue;if(el.closest&&el.closest("[data-keyo-price-table]"))continue;var t=(el.textContent||"").replace(/\\s+/g," ").trim();if(!t||t.length<24||t.length>420)continue;if(t===want)continue;if(!BAD.test(t))continue;el.textContent=want;return}}
function tick(){try{rewriteListCards();var roots=detailRoots();if(!roots.length)roots=[document.body];for(var i=0;i<roots.length;i++){var root=roots[i];var mid=detect(root);if(!mid)continue;var u=meta(mid);if(!u)continue;rewrite(root,u);if(u.price_table){mountTable(root,mid,u);fixDesc(root,u)}}}catch(e){}}
var _t=null;function schedule(){if(_t)return;_t=setTimeout(function(){_t=null;tick()},80)}
setInterval(tick,350);try{new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true})}catch(e){}
