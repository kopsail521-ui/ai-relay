...
function buildBillingUnitScript() {
  // 1) Rewrite 按次→按秒 via text nodes (detail drawer + cards)
  // 2) Inject price_table under 定价 / Base Price (like competitor sites)
  const js = `(function(){if(window.__keyoBillingUnitV3)return;window.__keyoBillingUnitV3=1;
function map(){return window.__KEYO_MKT_COPY||{}}
var LANGS=["zhCN","zhTW","en","fr","ru","ja","vi"];
function langCode(){try{var v=(localStorage.getItem("i18nextLng")||"").trim().replace(/_/g,"-");var l=v.toLowerCase();if(LANGS.indexOf(v)>=0)return v;if(l==="zhcn"||l==="zh-cn"||l==="zh-hans"||l==="zh")return "zhCN";if(l==="zhtw"||l==="zh-tw"||l==="zh-hk"||l==="zh-mo"||l.indexOf("zh-hant")===0)return "zhTW";if(l.indexOf("ja")===0)return "ja";if(l.indexOf("fr")===0)return "fr";if(l.indexOf("ru")===0)return "ru";if(l.indexOf("vi")===0)return "vi";if(l.indexOf("en")===0)return "en";return "zhCN"}catch(e){return "zhCN"}}
function unitOf(n){return n?map()[n]:null}
function label(u,field){if(!u)return"";var code=langCode();var bag=u[field]||{};if(typeof bag==="string")return bag;return bag[code]||bag.zhCN||bag.en||""}
function detectModel(){try{var path=location.pathname||"";var m=path.match(/\\/(?:pricing|models)\\/(?:detail\\/)?([^\\/?#]+)/i);if(m){var id=decodeURIComponent(m[1]);if(unitOf(id))return id}var q=new URLSearchParams(location.search||"");var qm=q.get("model")||q.get("name");if(qm&&unitOf(qm))return qm;var keys=Object.keys(map()).filter(function(k){return k!=="__tags__"});var hs=document.querySelectorAll("h1,h2,h3");for(var i=0;i<hs.length;i++){var t=(hs[i].textContent||"").replace(/\\s+/g," ").trim();if(unitOf(t))return t;for(var j=0;j<keys.length;j++){if(t===keys[j]||t.indexOf(keys[j])>=0)return keys[j]}}}catch(e){}return null}
function rewriteTextNodes(root,u){if(!root||!u)return;if(u.unit==="token")return;var badge=label(u,"badge");var pkey=label(u,"price_key");var suf=label(u,"suffix");var tw=document.createTreeWalker(root,NodeFilter.SHOW_TEXT,null);var node;while((node=tw.nextNode())){var t=(node.nodeValue||"").replace(/\\s+/g," ").trim();if(!t)continue;if(u.unit!=="request"){if(/^(按次计费|按次計費|Per Request|Per-call|Par requête|За запрос|リクエスト課金|Theo lần gọi)$/.test(t)&&badge){node.nodeValue=badge;continue}if(/^(每次请求|每次請求|Per request|Par requête|За запрос|リクエストごと|Mỗi lần gọi)$/.test(t)&&pkey){node.nodeValue=pkey;continue}}if(suf&&/\\/\\s*(request|次|回|lần)$/i.test(t)&&u.unit==="second"){node.nodeValue=t.replace(/\\/\\s*(request|次|回|lần)$/i,suf)}}}
function findPricingSection(root){var all=root.querySelectorAll("h1,h2,h3,h4,h5,div,span,p");for(var i=0;i<all.length;i++){var el=all[i];if(el.children&&el.children.length>3)continue;var t=(el.textContent||"").replace(/\\s+/g," ").trim();if(t==="定价"||t==="定價"||t==="Pricing"||t==="基础价格"||t==="基礎價格"||t==="Base Price"){var sec=el.closest("section")||el.parentElement;return sec}}return null}
function ensureTable(mid,u){if(!u||!u.price_table||!u.price_table.rows||!u.price_table.rows.length)return;var root=document.querySelector("main")||document.body;var sec=findPricingSection(root);if(!sec)return;var id="keyo-price-table-"+mid.replace(/[^a-zA-Z0-9_-]/g,"_");var old=document.getElementById(id);if(old)return;var wrap=document.createElement("div");wrap.id=id;wrap.setAttribute("data-keyo-price-table","1");wrap.style.cssText="margin-top:12px;overflow:auto;border:1px solid rgba(0,0,0,.08);border-radius:10px";var table=document.createElement("table");table.style.cssText="width:100%;border-collapse:collapse;font-size:13px";var thead=document.createElement("thead");var hr=document.createElement("tr");(u.price_table.columns||[]).forEach(function(c){var th=document.createElement("th");th.textContent=c;th.style.cssText="text-align:left;padding:10px 12px;background:rgba(0,0,0,.03);border-bottom:1px solid rgba(0,0,0,.08);font-weight:600";hr.appendChild(th)});thead.appendChild(hr);table.appendChild(thead);var tb=document.createElement("tbody");u.price_table.rows.forEach(function(row){var tr=document.createElement("tr");row.forEach(function(cell,idx){var td=document.createElement("td");td.textContent=cell;td.style.cssText="padding:10px 12px;border-bottom:1px solid rgba(0,0,0,.06)"+(idx===row.length-1?";font-family:ui-monospace,monospace;font-weight:600":"");tr.appendChild(td)});tb.appendChild(tr)});table.appendChild(tb);wrap.appendChild(table);sec.appendChild(wrap);try{var kids=sec.querySelectorAll("div,span");for(var i=0;i<kids.length;i++){var k=kids[i];if(k.closest("[data-keyo-price-table]"))continue;var tt=(k.textContent||"").replace(/\\s+/g," ").trim();if(/^(每次请求|每秒|Per request|Per second)$/.test(tt)||/^\\$\\d/.test(tt)&&k.parentElement&&/每次请求|Per request/.test((k.parentElement.textContent||""))){var row=k.closest(".flex")||k.parentElement;if(row&&!row.getAttribute("data-keyo-hide")){row.style.display="none";row.setAttribute("data-keyo-hide","1")}}}}catch(e){}}
function fix(){try{var mid=detectModel();var u=mid?unitOf(mid):null;var root=document.querySelector("main")||document.body;if(u){rewriteTextNodes(root,u);ensureTable(mid,u)}var keys=Object.keys(map()).filter(function(k){return k!=="__tags__"});var nodes=document.querySelectorAll("h1,h2,h3,h4,div,span,p,a,button");for(var i=0;i<nodes.length;i++){var el=nodes[i];if(el.children&&el.children.length>2)continue;var name=(el.textContent||"").replace(/\\s+/g," ").trim();if(!unitOf(name))continue;var scope=el.closest("[class*=card],article,section,li,div")||el.parentElement;if(scope)rewriteTextNodes(scope,unitOf(name))}}catch(e){}}
setInterval(fix,400);document.addEventListener("click",function(){setTimeout(fix,60)},true);window.addEventListener("popstate",function(){setTimeout(fix,60)});
})();`;
  return "<script>" + js + "</script>";
}
...
async function proxyRequest(req, res, bodyBuf) {
  ...
  if (isSpaShell && buf.length > 0) {
    try {
      ...
      // v3: 排序劫持 + 出海简介/计费单位；旧标记一并清掉后重注
      if (!html.includes("keyo-pricing-sort-v8")) {
        html = html
          .replace(/<!--keyo-pricing-sort(?:-v\d+)?-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-billing-unit-->[\s\S]*?<\/script>/g, "")
          .replace(/<!--keyo-locale-desc-->[\s\S]*?<\/script>/g, "");
        const inject = `<!--keyo-pricing-sort-v8-->${PRICING_SORT_SCRIPT}<!--keyo-locale-desc-->${LOCALE_DESC_SCRIPT}<!--keyo-billing-unit-->${BILLING_UNIT_SCRIPT}`;
        if (html.includes("<head>")) {
